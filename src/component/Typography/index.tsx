...
export function Typography(props: TypographyProps): JSX.Element {
    const scaleStyles = useMemo(() => {
        ...
        if (props.scale === 'label' || scale === undefined) scale = typescales.label
    ...
    return (
        <span className={classNames(
            css({
                ...scaleStyles,
                color: props.color === undefined ? hexFromArgb(getScheme().onSurface) :
                    typeof (props.color) === "number" ? hexFromArgb(props.color) : props.color
            })
        )
        }>
            {props.children}
